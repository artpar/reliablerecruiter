...
const STATIC_ASSETS = ['/', '/index.html', '/manifest.json', '/favicon.ico', '/pwa-192x192.png', '/pwa-512x512.png', '/apple-touch-icon.png',];

// PDF library resources to cache separately
const PDF_LIB_ASSETS = [// PDF.js files
    'https://unpkg.com/pdfjs-dist@5.1.91/build/pdf.min.mjs', 'https://unpkg.com/pdfjs-dist@5.1.91/build/pdf.worker.min.mjs',];

// Worker files to cache
const WORKER_ASSETS = [// PDF Worker files
    '/assets/pdfWorker.js', '/assets/workers/pdfWorker.js', '/src/workers/pdfWorker.js',

    // PDF Annotation Worker files
    '/assets/workers/pdfAnnotationWorker.js', '/src/workers/pdfAnnotationWorker.js',

    // PDF Editor Worker files
    '/assets/workers/pdfEditorWorker.js', '/src/workers/pdfEditorWorker.js'];

// Install event - Cache static assets
self.addEventListener('install', (event) => {
    self.skipWaiting(); // Force new service worker to become active immediately

    event.waitUntil(Promise.all([// Cache static assets
...
self.addEventListener('activate', (event) => {
    event.waitUntil(caches.keys().then(cacheNames => {
        return Promise.all(cacheNames.map(cache => {
            ...
            if (cache !== STATIC_CACHE && cache !== DYNAMIC_CACHE && cache !== PDF_LIB_CACHE && cache.includes('hr-toolkit')) {
...
// PDF-specific functions have been moved to dedicated workers in src/workers/ file
const isWorkerAsset = (url) => {
    return url.includes('/workers/') || url.includes('pdfWorker') || url.includes('/assets/workers/') || WORKER_ASSETS.some(asset => url.includes(asset));
};
...
const isNetworkFirstRequest = (url) => {
    return url.includes('hot-update') || url.includes('sockjs-node') || url.includes('dev-server') || url.includes('webpack');
};

// Helper to check content type
const isJavaScriptContentType = (response) => {
    const contentType = response.headers.get('content-type');
    return contentType && (contentType.includes('javascript') || contentType.includes('application/x-javascript') || contentType.includes('text/javascript'));
};
    ...
    if (url.endsWith('.js') && !isJavaScriptContentType(response)) {
        ...
        return response.blob().then(blob => {
            return new Response(blob, {
                status: response.status, statusText: response.statusText, headers: new Headers({
                    'Content-Type': 'application/javascript', // Copy other headers
...
self.addEventListener('fetch', (event) => {
    ...
    if (event.request.method !== 'GET' || requestUrl.startsWith('chrome-extension') || requestUrl.startsWith('moz-extension')) {
        return;
    }
    ...
    if (isPdfLibAsset(requestUrl)) {
        event.respondWith(caches.open(PDF_LIB_CACHE).then(cache => {
...
self.addEventListener('message', (event) => {
    console.log('[WHY] Received message', event);
});
